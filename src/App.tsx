...
async function callClaude(system: string, messages: { role: string; content: string }[]) {
  ...
  return (data.content as Array<{ type: string; text: string }>)?.find((b) => b.type === "text")?.text || "";
}
...
const fallback = () => "You already know this: the connection you seek was never lost. Take a breath. What did you come here to remember?";
...
export default function App() {
  ...
  async function startChat(p: Profile) {
    ...
    setLoading(true);
    try {
      const reply = await callClaude(buildSystem(p), [{
        role: "user",
        content: `This is the user's very first reflection. Based on their profile — role: "${p.role}", disconnects from: "${p.disconnect}", seeks: "${p.seeking}", pattern: "${p.wound}" — offer them a deeply personal, poetic opening reflection that feels like their higher self welcoming them home. End with one open question that invites them to go deeper.`,
      }]);
      setMessages([{ role: "assistant", content: reply || fallback() }]);
  ...
  async function sendMessage() {
    ...
    setLoading(true);
    try {
      const reply = await callClaude(buildSystem(profile), newMsgs.map((m) => ({ role: m.role, content: m.content })));
      setMessages((m) => [...m, { role: "assistant", content: reply || fallback() }]);
      if (!isPro) setReflectionsLeft((r) => Math.max(0, r - 1));
    } catch { setMessages((m) => [...m, { role: "assistant", content: fallback() }]); }
    setLoading(false);
  }
  ...
  const btnStyle = (extra?: CSSProperties): CSSProperties => ({ background: C.green, color: "#07111C", border: "none", padding: "13px 36px", borderRadius: 40, fontSize: 15, fontWeight: 700, fontFamily: "'Cinzel', serif", letterSpacing: "0.06em", cursor: "pointer", ...extra });
  const ghostStyle = (extra?: CSSProperties): CSSProperties => ({ background: "transparent", color: C.muted, border: `1px solid ${C.border}`, padding: "11px 24px", borderRadius: 40, fontSize: 14, fontFamily: "'Lora', serif", cursor: "pointer", ...extra });
  const optBtnStyle = (active: boolean, yellow = false): CSSProperties => ({ background: active ? (yellow ? "rgba(246,201,14,0.1)" : "rgba(63,182,139,0.1)") : "rgba(13,27,42,0.8)", border: `1px solid ${active ? (yellow ? C.yellow : C.green) : C.border}`, color: C.sand, padding: "12px 16px", borderRadius: 12, fontSize: 14, fontFamily: "'Lora', serif", textAlign: "left", cursor: "pointer", lineHeight: 1.4, transition: "all 0.2s" });
  ...
  if (screen === "splash") return (
    <div style={root}>
      ...
      <div style={{ zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", animation: "fadeUp 1s ease both", padding: "0 24px", textAlign: "center" }}>
        ...
        <p style={{ color: C.muted, maxWidth: 320, lineHeight: 1.75, marginBottom: 40 }}>Not a guru. Not a therapist. A permission slip to access the answers already within.</p>
        <button style={btnStyle()} onClick={() => setScreen("intake")}>Begin</button>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                {currentQ.opts.map((opt) => (
                  <button key={opt} onClick={() => selectFirst(opt)} style={optBtnStyle(sel.first === opt)}>
              ...
              {sel.first && (
                <div style={{ marginTop: 20, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <button onClick={() => setPickingSecond(true)} style={ghostStyle({ fontSize: 13 })}>+ Add second choice</button>
                  <button onClick={nextStep} style={btnStyle()}>{step < INTAKE.length - 1 ? "Continue →" : "Enter →"}</button>
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                {currentQ.opts.filter((o) => o !== sel.first).map((opt) => (
                  <button key={opt} onClick={() => selectSecond(opt)} style={optBtnStyle(sel.second === opt, true)}>
                    {sel.second === opt && <span style={{ color: C.yellow, marginRight: 8 }}>◈</span>}{opt}
                  </button>
                ))}
              </div>
              <div style={{ marginTop: 20, display: "flex", gap: 12 }}>
                <button onClick={() => setPickingSecond(false)} style={ghostStyle()}>← Back</button>
                <button onClick={nextStep} style={btnStyle()}>{step < INTAKE.length - 1 ? "Continue →" : "Enter →"}</button>
              </div>
            </>
          )}
  ...
  if (screen === "chat") return (
    <div style={{ ...root, justifyContent: "flex-start", height: "100vh", overflow: "hidden" }}>
      <Starfield />
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 10, background: "rgba(7,17,28,0.92)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}`, padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={resetApp} style={ghostStyle({ padding: "6px 14px", fontSize: 12 })}>Reset</button>
        </div>
      </div>
        ...
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 20 }}>
              ...
              <div style={{ maxWidth: "78%", padding: "16px 20px", borderRadius: m.role === "user" ? "20px 20px 4px 20px" : "4px 20px 20px 20px", background: m.role === "user" ? "rgba(63,182,139,0.15)" : C.card, border: m.role === "user" ? "1px solid rgba(63,182,139,0.3)" : `1px solid ${C.border}`, color: C.sand, fontSize: 16, lineHeight: 1.85, fontFamily: "'Lora', Georgia, serif" }}>
                {m.content}
              </div>
            </div>
          ))}
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 10, background: "rgba(7,17,28,0.95)", backdropFilter: "blur(12px)", borderTop: `1px solid ${C.border}`, padding: "16px 24px", width: "100%" }}>
        {!isPro && reflectionsLeft <= 0 ? (
          <div style={{ textAlign: "center" }}>
            ...
            <button onClick={() => setScreen("upgrade")} style={btnStyle()}>Unlock More →</button>
          </div>
        ) : (
    ...
    <div style={root}>
      ...
      <div style={{ zIndex: 1, display: "flex", flexDirection: "column", alignItems: "center", width: "100%", maxWidth: 440, padding: "40px 24px", animation: "fadeUp 0.5s ease both" }}>
        ...
        <div style={{ width: "100%", background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px 24px", marginBottom: 16 }}>
          ...
          {!emailSent ? (
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={unlockWithEmail} style={btnStyle({ padding: "10px 20px", fontSize: 14 })}>Unlock</button>
            </div>
        ...
        <div style={{ width: "100%", background: C.card, border: "1px solid rgba(246,201,14,0.35)", borderRadius: 16, padding: "20px 24px" }}>
          ...
          <button disabled style={btnStyle({ marginTop: 12, opacity: 0.5, cursor: "default" })}>Join Waitlist</button>
        </div>
        <button onClick={() => setScreen("chat")} style={ghostStyle({ marginTop: 20 })}>← Back to Reflections</button>
      </div>
    </div>
  );
}