...
export default function Reflections() {
  const [screen, setScreen] = useState("splash"); // splash | intake | chat | upgrade | pro
  const [step, setStep] = useState(0);
  const [profile, setProfile] = useState({});
  const [choices, setChoices] = useState({}); // { [id]: { first, second } }
  ...
  function selectFirst(opt) {
    setChoices((c) => ({ ...c, [currentQ.id]: { first: opt, second: null } }));
    setPickingSecond(false);
  }

  function selectSecond(opt) {
    if (opt === choices[currentQ.id]?.first) return; // can't pick same
    setChoices((c) => ({
      ...c,
      [currentQ.id]: { ...c[currentQ.id], second: opt },
    }));
  }

  function nextStep() {
    if (!choices[currentQ.id]?.first) return;
    if (step < INTAKE.length - 1) {
      setStep((s) => s + 1);
      setPickingSecond(false);
    } else {
      // Build profile from first choices
      const p = {};
      INTAKE.forEach((q) => {
        p[q.id] = choices[q.id]?.first || "";
      });
      setProfile(p);
      startChat(p);
    }
  ...
  // ── Start chat with personalized first reflection ──────────────────
  async function startChat(p) {
    setScreen("chat");
    setLoading(true);
    const systemPrompt = buildSystem(p);
    const firstMsg = buildFirstReflection(p);
    try {
      ...
      setMessages([
        {
          role: "assistant",
          content:
            "You already know this: the connection you seek was never lost. Take a breath. Ask what you came here to remember.",
        },
      ]);
    }
    setLoading(false);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages((m) => [
        ...
        {
          role: "assistant",
          content: "You already know this: pause here. The answer is closer than it seems.",
        },
      ]);
    }
    setLoading(false);
...
Let this profile inform your tone and what you reflect back — subtly, naturally.`;
  }

  function buildFirstReflection(p) {
    return `This is the user's very first reflection. They just completed their intake. Based on their profile — role: "${p.role}", disconnects from: "${p.disconnect}", seeks: "${p.seeking}", pattern: "${p.wound}" — offer them a deeply personal, poetic opening reflection that feels like their higher self welcoming them home. This sets the entire tone of the relationship. Make it land. End with one open question that invites them to go deeper.`;
  }

  // ── Email unlock (simulated) ──────────────────────────────────────
  function unlockWithEmail() {
    if (!email.includes("@")) return;
    setEmailSent(true);
    setTimeout(() => {
      setIsPro(false);
      setReflectionsLeft(FREE_LIMIT); // reset free for demo
      setEmailSent(false);
      setEmail("");
      setScreen("chat");
    }, 1500);
    ...
    return (
      <div style={styles.root}>
        ...
        <div style={styles.splashInner}>
          ...
          <h1 style={styles.splashTitle}>Reflections</h1>
          <p style={styles.splashSub}>A mirror for the soul.</p>
          <p style={{ ...styles.muted, maxWidth: 320, textAlign: "center", lineHeight: 1.7, marginBottom: 40 }}>
            Not a guru. Not a therapist. A permission slip to access the answers already within.
          </p>
          <button style={styles.btnPrimary} onClick={() => setScreen("intake")}>
            Begin
          </button>
          <p style={{ ...styles.muted, fontSize: 13, marginTop: 16 }}>Free · 2 reflections</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={styles.root}>
        ...
        <div style={styles.intakeWrap}>
          ...
          <p style={{ ...styles.muted, fontSize: 12, marginBottom: 28, letterSpacing: "0.08em" }}>
            {step + 1} of {INTAKE.length}
          </p>

          <h2 style={styles.intakeQ}>{q.q}</h2>

          {!pickingSecond ? (
            <>
              <p style={{ ...styles.muted, fontSize: 13, marginBottom: 20 }}>Choose what resonates most</p>
              <div style={styles.optGrid}>
                {q.opts.map((opt) => (
                  <button
                    ...
                    style={{
                      ...styles.optBtn,
                      ...(sel.first === opt ? styles.optSelected : {}),
                    }}
                    onClick={() => selectFirst(opt)}
                  >
                    {sel.first === opt && <span style={{ color: C.green, marginRight: 8 }}>✦</span>}
                    {opt}
                  </button>
                ))}
              </div>
              ...
              {sel.first && (
                <div style={{ marginTop: 20, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <button
                    style={{ ...styles.btnGhost, fontSize: 13 }}
            ...
            <>
              <p style={{ ...styles.muted, fontSize: 13, marginBottom: 8 }}>
                First choice: <span style={{ color: C.green }}>{sel.first}</span>
              </p>
              <p style={{ ...styles.muted, fontSize: 13, marginBottom: 20 }}>
                Now choose a second (optional — only your first shapes your experience)
              </p>
              <div style={styles.optGrid}>
                {q.opts
                  .filter((o) => o !== sel.first)
                  .map((opt) => (
                    <button
                      key={opt}
                      style={{
                        ...styles.optBtn,
                        ...(sel.second === opt ? styles.optSecond : {}),
                      }}
                      onClick={() => selectSecond(opt)}
                    >
                      {sel.second === opt && <span style={{ color: C.yellow, marginRight: 8 }}>◈</span>}
                      {opt}
                    </button>
                  ))}
              </div>
              <div style={{ marginTop: 20, display: "flex", gap: 12 }}>
                <button style={styles.btnGhost} onClick={() => setPickingSecond(false)}>
                  ← Back
                </button>
                <button style={styles.btnPrimary} onClick={nextStep}>
                  {step < INTAKE.length - 1 ? "Continue →" : "Enter →"}
  ...
  if (screen === "chat") {
    return (
      <div style={{ ...styles.root, justifyContent: "flex-start" }}>
        <Starfield />
        {/* Header */}
        <div style={styles.header}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: 22 }}>🌙</span>
            <span style={styles.headerTitle}>Reflections</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            {!isPro && (
              <span style={styles.badge}>
                {reflectionsLeft} reflection{reflectionsLeft !== 1 ? "s" : ""} left
              </span>
            )}
            {isPro && <span style={{ ...styles.badge, background: "rgba(246,201,14,0.15)", color: C.yellow }}>Pro</span>}
            <button style={styles.btnGhostSm} onClick={() => { setScreen("splash"); setMessages([]); setStep(0); setChoices({}); setReflectionsLeft(FREE_LIMIT); }}>
              Reset
            </button>
          </div>
        </div>
        ...
        <div style={styles.msgArea}>
          {messages.length === 0 && !loading && (
            <div style={{ textAlign: "center", marginTop: 80 }}>
              ...
              <p style={{ ...styles.muted, marginTop: 12 }}>Your reflection is being prepared…</p>
            </div>
          )}
          {messages.map((m, i) => (
            <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 20 }}>
              {m.role === "assistant" && (
                <div style={{ marginRight: 10, marginTop: 4, fontSize: 18, opacity: 0.7 }}>🌙</div>
              )}
              <div
                style={{
                  maxWidth: "75%",
          ...
          {loading && (
            ...
            <div style={{ display: "flex", gap: 5, padding: "14px 18px", background: C.card, border: `1px solid ${C.border}`, borderRadius: "4px 20px 20px 20px" }}>
                <span className="typing-dot" style={{ width:7,height:7,borderRadius:"50%",background:C.green,display:"inline-block",animation:"dotPulse 1.2s infinite ease-in-out" }} />
                <span className="typing-dot" style={{ width:7,height:7,borderRadius:"50%",background:C.green,display:"inline-block",animation:"dotPulse 1.2s 0.2s infinite ease-in-out" }} />
                <span className="typing-dot" style={{ width:7,height:7,borderRadius:"50%",background:C.green,display:"inline-block",animation:"dotPulse 1.2s 0.4s infinite ease-in-out" }} />
              </div>
            </div>
          )}
          <div ref={endRef} />
        ...
        <div style={styles.inputArea}>
          {!isPro && reflectionsLeft <= 0 ? (
            <div style={{ textAlign: "center" }}>
              <p style={{ color: C.yellow, marginBottom: 12, fontSize: 14 }}>
                You've used your free reflections for today.
              </p>
              <button style={styles.btnPrimary} onClick={() => setScreen("upgrade")}>
                Unlock More →
              </button>
            </div>
          ) : (
    ...
    return (
      <div style={styles.root}>
        ...
        <div style={styles.upgradeWrap}>
          ...
          <h2 style={styles.splashTitle}>Go Deeper</h2>
          <p style={{ ...styles.muted, maxWidth: 300, textAlign: "center", lineHeight: 1.7, marginBottom: 32 }}>
            You've used your free reflections. The conversation doesn't have to end here.
          </p>

          {/* Email unlock */}
          <div style={styles.upgradeCard}>
            <p style={{ color: C.sand, fontWeight: 600, marginBottom: 4 }}>Free — Unlock with email</p>
            <p style={{ ...styles.muted, fontSize: 13, marginBottom: 16 }}>Get 2 more reflections per day</p>
            {!emailSent ? (
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  style={{ ...styles.input, flex: 1, fontSize: 14, padding: "10px 14px" }}
                  placeholder="your@email.com"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                />
                <button style={styles.btnPrimary} onClick={unlockWithEmail}>
                  Unlock
                </button>
              </div>
            ) : (
          ...
          {/* Pro */}
          <div style={{ ...styles.upgradeCard, borderColor: "rgba(246,201,14,0.35)", marginTop: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <p style={{ color: C.sand, fontWeight: 600 }}>Pro — $9/mo</p>
              <span style={{ background: "rgba(246,201,14,0.15)", color: C.yellow, fontSize: 11, padding: "3px 8px", borderRadius: 20, letterSpacing: "0.06em" }}>
                COMING SOON
              </span>
            </div>
            <ul style={{ ...styles.muted, fontSize: 13, lineHeight: 2, paddingLeft: 0, listStyle: "none" }}>
              <li>✦ Unlimited reflections</li>
              <li>✦ Return to past sessions</li>
              <li>✦ Retake your intake anytime</li>
              <li>✦ Deeper reflection modes</li>
            </ul>
            <button style={{ ...styles.btnPrimary, opacity: 0.5, cursor: "default", marginTop: 8 }} disabled>
              Join Waitlist
            </button>
          </div>

          <button style={{ ...styles.btnGhost, marginTop: 20 }} onClick={() => setScreen("chat")}>
            ← Back to Reflections
          </button>
        </div>
      </div>
      ...
      {stars.map((s) => (
        <div
          ...
          style={{
            ...
            background: "white",
            opacity: 0.25,
            animation: `twinkle 3s ${s.animDelay} infinite alternate`,
          }}
        />
      ))}
      <style>{`
        ...
        div[style*="typingDots"] span, .typing-dot { width:7px;height:7px;border-radius:50%;background:#3FB68B;display:inline-block;animation:dotPulse 1.2s infinite ease-in-out; }
...
const styles = {
  ...
  moonGlow: {
    ...
    height: 200,
    borderRadius: "50%",
    background: "radial-gradient(circle, rgba(63,182,139,0.12) 0%, transparent 70%)",
    top: -60,
    pointerEvents: "none",
  },
  splashTitle: {
